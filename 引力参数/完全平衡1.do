...
*python处理
use D:\引力参数\回归数据\gra_raw.dta,clear
collapse (sum) industy_value,by(yr iso3 firmn)
rename industy_value v
save D:\引力参数\数据集\完全平衡\ex.dta,replace

use "D:\banlance\sample10000.dta",clear
drop index me
gen len = length(trim(iso3))
keep if len == 3
drop len
sort firmn yr
merge 1:1 firmn iso3 yr using D:\引力参数\数据集\完全平衡\ex.dta
drop if _merge == 2
drop _merge
save D:\引力参数\数据集\完全平衡\ex1.dta,replace

use D:\引力参数\数据集\完全平衡\ex1.dta,clear
drop if v == .
gen id = firmn
sort id yr
bysort id: keep if _n==1
keep firmn yr
rename yr fis_yr
save D:\引力参数\数据集\完全平衡\fir_yr.dta,replace
use D:\引力参数\数据集\完全平衡\ex1.dta,clear
merge n:1 firmn using D:\引力参数\数据集\完全平衡\fir_yr.dta
keep if _merge == 3
drop _merge
drop if 
save D:\引力参数\数据集\完全平衡\reg.dta,replace
